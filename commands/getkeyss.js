...
module.exports = {
    async run(bot, message, args) {
        ...
        if (message.channel.name !== "op-gen") return message.channel.send(new discord.RichEmbed()
            .setTitle(`Stop!`)
            .setDescription(`**You need to use the correct channel (<#${message.guild.channels.find(channel => channel.name === "op-gen").id}>).**`)
            .setColor(0xff0000));
        if (!message.member.roles.has(message.guild.roles.find(role => role.name === "| OP Gen Access").id)) return message.channel.send(new discord.RichEmbed()
            .setTitle(`Stop!`)
